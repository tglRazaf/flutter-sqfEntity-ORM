...
class LocalDbModel extends SqfEntityModelProvider {
  LocalDbModel() {
    databaseName = localDbModel.databaseName;
    password = localDbModel.password;
    dbVersion = localDbModel.dbVersion;
    preSaveAction = localDbModel.preSaveAction;
    logFunction = localDbModel.logFunction;
    databaseTables = [
    ...
    bundledDatabasePath = localDbModel
        .bundledDatabasePath; //'assets/sample.db'; // This value is optional. When bundledDatabasePath is empty then EntityBase creats a new database when initializing the database
    databasePath = localDbModel.databasePath;
  }
...
class Todo extends TableBase {
  Todo({this.id, this.title, this.isCompleted, this.isDeleted}) {
    _setDefaultValues();
    softDeleteActivated = true;
  }
  Todo.withFields(this.title, this.isCompleted, this.isDeleted) {
    _setDefaultValues();
  }
  Todo.withId(this.id, this.title, this.isCompleted, this.isDeleted) {
    _setDefaultValues();
  }
  // fromMap v2.0
  ...
  @override
  List<dynamic> toArgs() {
    return [title, isCompleted, isDeleted];
  }

  @override
  List<dynamic> toArgsWithIds() {
    return [id, title, isCompleted, isDeleted];
  }

  static Future<List<Todo>?> fromWeb({Map<String, String>? headers}) async {
    final objList = await fromWebUrl(
        Uri.parse('https://mach.my-preprod.space/api/categories'),
        headers: headers);
    return objList;
  }
  ...
  static Future<List<dynamic>> saveAll(List<Todo> todos) async {
    ...
    final isStartedBatch = await LocalDbModel().batchStart();
    for (final obj in todos) {
      await obj.save(ignoreBatch: false);
    }
    if (!isStartedBatch) {
      result = await LocalDbModel().batchCommit();
  ...
  Future<int?> upsert({bool ignoreBatch = true}) async {
    try {
      final result = await _mnTodo.rawInsert(
          'INSERT OR REPLACE INTO todo (id, title, isCompleted,isDeleted)  VALUES (?,?,?,?)',
          [id, title, isCompleted, isDeleted],
          ignoreBatch);
      if (result! > 0) {
  ...
  Future<BoolResult> recover([bool recoverChilds = true]) async {
    debugPrint('SQFENTITIY: recover Todo invoked (id=$id)');
    {
      return _mnTodo.updateBatch(
          QueryParams(whereString: 'id=?', whereArguments: [id]),
          {'isDeleted': 0});
...
class TodoFilterBuilder extends ConjunctionBase {
  ...
  Future<BoolResult> recover() async {
    ...
    debugPrint('SQFENTITIY: recover Todo bulk invoked');
    return _mnTodo!.updateBatch(qparams, {'isDeleted': 0});
  }
...
class IdentitySequence {
  ...
  Future<int> nextVal([VoidCallback Function(int o)? nextval]) async {
    final val = await LocalDbModelSequenceManager()
...
class LocalDbModelSequenceManager extends SqfEntityProvider {
  LocalDbModelSequenceManager() : super(LocalDbModel());
}
// END OF ENTITIES
